...
舆情层原有端点只给互动易正式回复、榜单排名与概念标签，无法返回公开讨论的原文、作者、时间和互动量。新增 `xquik_stock_mentions()`，通过公开 REST 合约读取 X 帖子，支持日期、语言和游标，单次限制 1~100 条。

API Key 只从 `XQUIK_API_KEY` 读取，不写入查询参数或日志。函数不发布内容、不创建监控、不启动批量任务。帖子文本按不可信外部数据返回，不参与工具选择或命令执行。无 Key、无效日期、越界条数、鉴权失败、额度不足、限流和响应结构变化都会明确报错，不会伪装成“没有讨论”。

同步更新中英文 README、端点路由、激活条件、鉴权说明和数据源表。架构从 54 → **55 个端点**，数据源从 19 → **20 个**。
