...
### 新增：「估值分析铁律」章节（§估值计算公式 → 估值分析的铁律）

新增"估值分析的铁律：每一步都要过计算"章节。总结从原始估值数据（PE/PB/EPS）
到描述性结论的过程中最常见的错误模式——**不是在计算环节出错，而是在数字和结论之间
跳过计算、凭感觉硬编"生动解释"**。

- **强制规则**：任何从估值数字推导出的解释性描述，必须从原始数字反推验证
- **常见陷阱表**：PB 方向搞反、混淆跌幅与绝对值、编造不可验证的类比
- **自查流程**：三步反向验算法则
- **核心原则**：不要让"寻找便宜信号"的动机篡改算术

案例来源：AI 在分析牧原股份(002714) PB=2.61 时，跳过 `1/2.61=0.38` 的计算步骤，
直接错写成"花1块钱买到1.5毛的净资产"（实际应为"每花1元买到约0.38元净资产"）。
