...
### 新增：「估值分析铁律」+ 估值转换函数（§估值计算公式）

- **铁律**：估值结论必须通过数字计算得出，禁止凭感觉编造。每句描述都必须能从原始数字直接算出来。
- **新增 5 个估值转换函数**（此前缺失，导致容易跳过计算直接编结论）：
  - `pb_breakdown(pb)` — PB → "每花X元买到Y元净资产"（杜绝 PB 方向搞反）
  - `earnings_yield(pe)` — PE → 盈利收益率百分比
  - `peg_check(pe, cagr)` — PEG 校验，禁止手写"便宜/贵"
  - `fair_price(eps_forecast, target_pe)` — 一致预期EPS × 合理PE → 合理股价
  - `fair_pb(bps, historical_median_pb)` — BPS × 历史中位PB → 合理PB估值价
