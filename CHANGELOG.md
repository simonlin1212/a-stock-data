...
22 个来源。新来源为中证指数、国证指数和北交所；沪深交易所已存在，本次增加其有效端点。

### Breaking Changes

无。旧函数签名和代码保持不变；新函数接受纯 6 位代码、显式指数提供方或交易所。
Python 3.9 兼容，`requests pandas xlrd openpyxl` 均为已有依赖；SKILL.md 仍自包含，测试和调研文档不是运行时依赖。

### 新增

- `index_constituents(index_code, provider="csi")`：中证/国证最近公布的沪深北成分，保留源侧快照日期。
- `index_weights(index_code, provider="csi")`：中证/国证权重，单位百分数，检验权重合计及重复记录。
- `index_valuation(index_code)`：中证近期两种股本口径的 PE、股息率，**不含 PB、不承诺全历史**。
- `trading_calendar(year, month)`：深交所整月日历，校验月份、日期完整性和交易标志，不自行猜节假日。
- `margin_trading_backup(trade_date, exchange, code=None)`：沪深官方两融，分别请求单所，金额元、余量股/份；上交所未给出的融券金额保留为空。
- `bse_quote_backup(trade_date, code=None)`：北交所全板/单票当前行情与五档快照，核验交易日和全部分页。

### 优化

- 新输出均带 `source`、`source_url`、`fetched_at`；`date` 区分源侧交易日/月末日/日历日期和抓取日。
- 国证港股五位代码直接拒绝，避免 `00700` 补零成 `000700/SZ` 后静默指向错误证券。
- 中英文 README 同步能力、计数、示例及边界；版本亮点移入 CHANGELOG，保留线上最新的作者机会栏目。
- 新增[数据源整合与验证记录](docs/source-integration-v3.8.0.md)：集合竞价、需 Key 的同花顺平台、TickFlow 等仅登记候选，不计入已接入来源。

### 测试

- 26 个离线测试全部通过，测试直接执行 SKILL.md 发布代码；补充两融返回市场身份校验、异常分页总数拒绝的先失败后修复回归。
- Python 3.9 实跑新增代码；离线测试直接从 SKILL.md 抽取发布实现，覆盖代码误路由、数值口径、缺列、空表、分页、旧快照、重定向、日历缺日及 HTTP 错误。
- 2026-09-05 真实接口验证：沪深300成分/权重各300行；创业板指成分/权重各100行；中证估值20行；9月日历30行；沪深两融分别2000/2103行；北交所全板341行及单票1行。
- 真实未发布边界：深交所周六 2026-09-05 和未来 2026-09-30 两融文件均只有表头，未退回最新交易日；程序据此报错。
- 旧端点没有全部重新联网验证，其既有验证日期仍按各章节记录；本版验证不代表第三方接口的长期可用性承诺。

## v3.7.2 — 2026-09-02

### 修复

**`_natural_market()` / `_anomaly_market()` 的北交所号段判定与 `get_prefix()` 不一致（#51）**

`get_prefix()` 用 `startswith(("4", "8"))` + `startswith("92")` 覆盖整个北交所号段，而
`_natural_market()`（`norm_ticker()` 校验显式前缀是否自相矛盾）和 §8.5 `_anomaly_market()`
（异动记录 → 交易所）只枚举了 `43 / 83 / 87` 三个前两位和 `920`：同一份文件里两套规则，
`norm_ticker("SZ400001")` 不报错、`get_prefix("400001")` 却返回 `bj`。现在三处统一为
`startswith(("4", "8", "92"))`。

**实际影响说明**（2026-09-01 实测）：北交所在市代码只有 `43x / 83x / 87x / 920x` 四段
（腾讯批量探测 `921000–921999` 全部为空，921 段尚未启用），所以这次修复**不改变任何现有
标的的判定结果**；它修的是一致性，并让 921 段启用后不必再改一次。感谢 @notylou 逐段对比。
